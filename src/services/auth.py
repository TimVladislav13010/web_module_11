...
"""
Service Authorization users.
"""


class Auth:
    """
    Class Authorization users.
    """
    ...
    def verify_password(self, plain_password, hashed_password):
        ...
        :param hashed_password: Compare the plain_password parameter to it
        :return: A boolean
        :doc-author: Trelent
        """
    ...
    async def create_refresh_token(self, data: dict, expires_delta: Optional[float] = None):
        ...
        :return: A refresh token, which is a jwt that contains the user's id and iat (issued at) and exp (expiration time)
        :doc-author: Trelent
        """
        to_encode = data.copy()
        if expires_delta:
    ...
    async def get_current_user(self, token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
        ...
        to retrieve the current user. It uses the oauth2_scheme to get an access token from
        the Authorization header in each request, and then validates it using PyJWT. If it's valid,
        it returns a User object with information about that user.
        ...
        #         raise credentials_exception
        #     user = user.email
        #     self.r.set(email, pickle.dumps(user))
        #     self.r.expire(email, 900)
        # else:
    ...
    def create_email_token(self, data: dict):
        ...
        :return: A token that can be used to confirm the user's email address
        :doc-author: Trelent
        """
        to_encode = data.copy()
